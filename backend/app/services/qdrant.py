...
from qdrant_client.models import VectorParams, Distance, Batch, Filter, FieldCondition
...
class QdrantService:
    ...
    def query_by_vector(self, query_vector: List[float], limit: int = 5, min_score: float = 0.0, filters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        q_filter = None
        if filters:
            must_conditions = []
            for k, v in filters.items():
                must_conditions.append(FieldCondition(key=k, match={"value": v}))
            if must_conditions:
            ...
            for hit in response.points:
                ...
                results.append({
                    ...
                    "metadata": {
                        ...
                        "chapter_title": payload.get("chapter_title") or payload.get("chapter") or payload.get("section"),
